var connectionStateMap, fs, parsePatterns, urlify, win32WirelessProfileBuilder;
...
connectionStateMap = {
  ...
  asociando: "connecting",
  autenticando: "authtenticating",
...
module.exports = {
  ...
  getIfaceState: function() {
    ...
    for (k = j = 0, len = ref.length; j < len; k = ++j) {
      ...
      try {
        ln_trim = ln.trim();
        if (ln_trim === "Software Desactivado") {
          interfaceState = {
            ssid: null,
            connected: false,
            power: false
      ...
      switch (KEY) {
        ...
        case "SSID":
          ...
          break;
        case "Estado de radio":
          if (VALUE === "Hardware Desactivado") {
            interfaceState = {
              ssid: null,
              connected: false,
              power: false
  ...
  connectToAP: function(_ap) {
    ...
    COMMANDS = {
      loadProfile: ("netsh " + this.WiFiControlSettings.iface + " add profile filename=\"") + urlify(_ap.ssid) + ".xml\"",
    ...
    return this.execSync("del \".\\" + urlify(_ap.ssid) + ".xml\"");
  },
  resetWiFi: function() {
